...
{
    /*SCRIPT EXTERN modificat per adaptar-se al nostre projecte (Canal de Youtube: Christina Creates Games. Link al vídeo: https://www.youtube.com/watch?v=UR_Rh0c4gbY */

    //script que serveix per mostrar un text que apareix lletra a lletra a la intro del joc.
    ...
    private IEnumerator Typewriter()
    {
        ...
        while (currentVisibleCharacterIndex < textInfo.characterCount + 1)
        {
            ...
            audioSource.Play();
            if (!estaSaltando && character == '.' || character == ',' || character == '?')
            {
